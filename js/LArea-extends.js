	...
	MobileArea.prototype = {
		...
		bindEvent: function() {
			...
			function popupArea(e) {
				...
				larea_cancel.addEventListener(touchEvents.touchstart, function(e) {
					_self.close(e);
				});
				var larea_finish = _self.gearArea.querySelector(".larea_finish");
				larea_finish.addEventListener(touchEvents.touchstart, function(e) {
					_self.finish(e);
				});
				...
				area_province.addEventListener(touchEvents.touchstart, gearTouchStart);
				area_city.addEventListener(touchEvents.touchstart, gearTouchStart);
				area_county.addEventListener(touchEvents.touchstart, gearTouchStart);
				area_province.addEventListener(touchEvents.touchmove, gearTouchMove);
				area_city.addEventListener(touchEvents.touchmove, gearTouchMove);
				area_county.addEventListener(touchEvents.touchmove, gearTouchMove);
				area_province.addEventListener(touchEvents.touchend, gearTouchEnd);
				area_city.addEventListener(touchEvents.touchend, gearTouchEnd);
				area_county.addEventListener(touchEvents.touchend, gearTouchEnd);
			...
			function gearTouchStart(e) {
				...
				var distance = typeof(e.y) != 'undefined' ? e.y : e.targetTouches[0].screenY;
				clearInterval(target["int_" + target.id]);
				target["old_" + target.id] = distance;
			...
			function gearTouchMove(e) {
				...
				var distance = typeof(e.y) != 'undefined' ? e.y : e.targetTouches[0].screenY;
				target["new_" + target.id] = distance;
			...
			function gearTouchEnd(e) {
				...
				var flag = (target["new_" + target.id] - target["old_" + target.id]) / (target["n_t_" + target.id] - target[
					"o_t_" + target.id]);
		...
		close: function(e) {
			...
			//$("body").find("div.gearArea").remove()
			var divs=document.querySelectorAll("div.gearArea");
